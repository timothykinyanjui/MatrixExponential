clear
% Load the data
load ModelRunsReplicates_N_SSSD
D = matsize(:,:,1);
...
% outlinebounds(hl,hp);
%hand = plot(D,meanTol(:,1),'r',D,meanTol(:,2),'b',D,meanTol(:,3),'g',D,meanTol(:,4),'k',D,meanTol(:,5),'m',D,meanTol(:,6),'c',D,meanTol(:,7),'y');
...
title('Accuracy - SSSD'); ylabel('log(SSSD)'); xlabel('Matrix size')
set(hand,'LineWidth',1.5);
hand = legend('ODE4','DA-1','Cheby','KSA','DA-2','Mohy','DA-3','Mohy2010','Location','Best'); set(hand,'Box','off')
box off; set(gca,'XTick',0:250:4200,'XtickLabelRotation',45); hand = text(500,1,'A'); set(hand,'FontWeight','bold')
...
subplot(1,2,2)
%[hl, hp] = boundedline(D,meanTime(:,1),[lb(:,1) ub(:,1)],'r','alpha',D,meanTime(:,2),[lb(:,2) ub(:,2)],'b','alpha',D,meanTime(:,3),[lb(:,3) ub(:,3)],'g','alpha',D,meanTime(:,4),[lb(:,4) ub(:,4)],'k','alpha',D,meanTime(:,5),[lb(:,5) ub(:,5)],'m','alpha',D,meanTime(:,6),[lb(:,6) ub(:,6)],'c','alpha',D,meanTime(:,7),[lb(:,7) ub(:,7)],'y','alpha');
...
%outlinebounds(hl,hp);
title('Computational time'); ylabel('Time (log seconds)'); xlabel('Matrix size')
set(gca,'XTick',0:250:4200,'XtickLabelRotation',45);  hand = text(500,9,'B'); set(hand,'FontWeight','bold')